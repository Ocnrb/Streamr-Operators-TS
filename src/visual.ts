...
// --- CONFIG ---
const DEFAULT_GRAPH_API_KEY = 'bb77dd994c8e90edcbd73661a326f068';
const SUBGRAPH_ID = 'EGWFdhhiWypDuz22Uy7b3F69E9MEkyfU9iAQMttkH5Rj';

function getGraphUrl() {
    const storedKey = localStorage.getItem('streamr_explorer_thegraph_api_key');
...
export const VisualLogic = {
    ...
    dragEnded: function(event) {
        ...
        if (this.isLiveMode && event.subject.type === 'sponsorship') {
            event.subject.fx = event.x; event.subject.fy = event.subject.y;
        } else {
    ...
    toggleLiveMode: async function(activate) {
        ...
        const delegatorSettings = document.getElementById('vis-delegator-settings-container');
        const delegatorCounter = document.getElementById('vis-delegator-counter'); // Element might not exist in HTML, check IDs
    ...
    processData: function(sponsorships, blockNumber) {
        ...
        sponsorships.forEach(s => {
            ...
            s.stakes.forEach(stake => {
                ...
                let currentMeta = this.operatorNodeMetadata.get(opId) || {};
                const sourceNode = operatorNodeExists ? nodesMap.get(opId) : currentMeta;
        ...
        if (!this.isLiveMode) {
            const opCount = newNodes.filter(n => n.type === 'operator').length;
            let displayText = `${opCount} OPERATORS`;
            
            if (this.isShowDelegators) {
                const delCount = newNodes.filter(n => n.type === 'delegator').length;
    ...
    requestRender: function() {
        if (!this.renderRequested) {
            ...
            requestAnimationFrame(() => {
                ...
                if (this.isLiveMode && this.isActive) {
                    this.requestRender();
                }
            });
        }
