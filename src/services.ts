...
export async function confirmDelegation(signer, myRealAddress, currentOperatorId) {
    ...
    if (/[eE]/.test(amount) || parseFloat(amount) > parseFloat(MAX_DELEGATION_AMOUNT)) {
        showCustomAlert('Invalid Amount', 'Please enter a reasonable amount without scientific notation.');
        return null;
    }
    ...
    try {
        ...
        try {
            ...
            showCustomAlert('Invalid Amount', 'Could not parse the amount. Please enter a valid number.');
            setModalState('tx-modal', 'input');
            return null;
        }
